...
def repo_issue_and_repo_issue_comment_format(repo_issue_and_repo_issue_comment):
    ...
    for issue in repo_issue_and_repo_issue_comment:
        ...
        issue = {'repo_id': data['repository']['id'], 'issue_id': data['id'], 'issue_number': data['number'], 'create_time': time_format(data['created_at']), 'user_id': data['user']['id'], 'author_association': None, 'issue_state': data['issue_state']}
        repo_issue.append(issue)

        if 'comments_data' in data.keys():
            for comment in data['comments_data']:
                issue_comment = {'repo_id': data['repository']['id'], 'issue_number': data['number'], 'issue_comment_id': comment['id'], 'create_time': time_format(comment['created_at']), 'user_id': comment['user']['id']}
...
def repo_pull_and_repo_pull_merged_and_repo_review_comment_format(repo_pull_and_repo_pull_merged_and_repo_review_comment):
    ...
    for pull in repo_pull_and_repo_pull_merged_and_repo_review_comment:
        ...
        pr = {'repo_id': data['base']['repo']['id'], 'pull_id': data['id'], 'pull_number': data['number'], 'create_time': time_format(data['created_at']), 'merge_time': time_format(data['merged_at']), 'pull_state': data['state'], 'user_id': data['user']['id']}
        repo_pull.append(pr)
        
        if data['merged_at'] is not None:
            pull_merged = {'repo_id': data['base']['repo']['id'], 'pull_id': data['id'], 'create_time': time_format(data['created_at']), 'merge_time': time_format(data['merged_at']), 'user_id': data['user']['id']}
            repo_pull_merged.append(pull_merged)

        if 'review_comments_data' in data.keys():
            for comment in data['review_comments_data']:
                review_comment = {'repo_id': data['base']['repo']['id'], 'pull_id': data['id'], 'review_comment_id': comment['id'], 'create_time': time_format(comment['created_at']), 'user_id': comment['user']['id']}
